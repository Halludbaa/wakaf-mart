...
class Transaksi extends BaseController
{
    ...
    public function bayarBank()
    {
        ...
        $dataTransaksi = [
            ...
            'id_bank' => (int) $this->request->getVar('id_bank'),
            'status' => 'pending',
            'order_id' => $this->request->getVar('order_id'),
        ];
        $this->MdlTransaksi->save($dataTransaksi);
        // Perbarui perolehan donasi di data_donasi
        $this->MdlDonasi->updatePerolehanDonasi($data['id_donasi'], $data['jumlah_donasi']);

        // Tampilkan halaman konfirmasi transfer
        return view('transaksi_bank', [
            'title' => 'Pembayaran Bank',
            'data' => $data,
            'donasi' => $this->MdlDonasi->find($data['id_donasi']),
            'bank' => $this->MdlBank->find($data['id_bank']),
        ]);
    }
    ...
    public function bayarQris()
    {
        ...
        // Tampilkan QR dan instruksi bayar
        return view('transaksi_qris', [
    ...
    public function success()
    {
        ...
        return view('transaksi_success', $data);
    }
}
